...
static void bpf_common_lru_populate(struct bb_bpf_lru *lru, void *buf,
				    u32 node_offset, u32 elem_size,
				    u32 nr_elems)
{
	struct bb_bpf_lru_list *l = &lru->common_lru.lru_list;
	u32 i;

	for (i = 0; i < nr_elems; i++) {
		struct bb_bpf_lru_node *node;

		node = (struct bb_bpf_lru_node *)(buf + node_offset);
		node->type = BPF_LRU_LIST_T_FREE;
		node->ref = 0;
		bb_list_add(box(&node->list), box(&l->lists[BPF_LRU_LIST_T_FREE]));
		buf += elem_size;
	}
}

static void bb_bpf_lru_populate(struct bb_bpf_lru *lru, void *buf, u32 node_offset,
...
static void bpf_lru_locallist_init(struct bb_bpf_lru_locallist *loc_l, int cpu)
{
	int i;

	for (i = 0; i < NR_BPF_LRU_LOCAL_LIST_T; i++)
		BB_INIT_LIST_HEAD(box(&loc_l->lists[i]));
...
static void bpf_lru_list_init(struct bb_bpf_lru_list *l)
{
	int i;

	for (i = 0; i < NR_BPF_LRU_LIST_T; i++)
		BB_INIT_LIST_HEAD(box(&l->lists[i]));

	for (i = 0; i < NR_BPF_LRU_LIST_COUNT; i++)
		l->counts[i] = 0;

	l->next_inactive_rotation = box(&l->lists[BPF_LRU_LIST_T_INACTIVE]);

	raw_spin_lock_init(&l->lock);
}

static int bb_bpf_lru_init(struct bb_bpf_lru *lru, bool percpu, u32 hash_offset,
		 bb_del_from_htab_func del_from_htab, void *del_arg)
{
	int cpu;
	...
	if (percpu) {
		...
		struct bb_bpf_common_lru *clru = &lru->common_lru;
		struct bb_bpf_lru_locallist *local_list = \
			bpfbox_alloc(sizeof(struct bb_bpf_lru_locallist) * nr_cpu_ids);

		if (!local_list)
			return -ENOMEM;

		for_each_possible_cpu(cpu) {
			struct bb_bpf_lru_locallist *loc_l;

			loc_l = &local_list[cpu];
			bpf_lru_locallist_init(loc_l, cpu);
		}

		clru->local_list = box(local_list);
...
static void bb_bpf_lru_destroy(struct bb_bpf_lru *lru)
{
	if (lru->percpu)
		BUG();
	else
		bpfbox_free(unbox(lru->common_lru.local_list));
}
...
static inline void bb_bpf_lru_node_set_ref(struct bb_bpf_lru_node *node)
{
...
static void htab_init_buckets(struct bpf_htab *htab)
{
	...
	for (i = 0; i < htab->n_buckets; i++) {
		BB_INIT_HLIST_NULLS_HEAD(&htab_inner(htab)->buckets[i].head, i);
				   ...
				   unsigned long *pflags)
{
	...
	/* if (unlikely(__this_cpu_inc_return(*(htab_inner(htab)->map_locked[hash])) != 1)) { */
	/* 	__this_cpu_dec(*(htab->map_locked[hash])); */
	/* 	preempt_enable(); */
	/* 	return -EBUSY; */
	/* } */

	lock = unbox(&(*unbox(&htab_inner(htab)->map_locked[cpu]))[hash]);
...
static bool htab_lru_map_delete_node(void *arg, struct bb_bpf_lru_node *node);
 ...
 * bb_bpf_lru_pop_free(), which is called by function prealloc_lru_pop(),
...
static struct htab_elem *prealloc_lru_pop(struct bpf_htab *htab, void *key,
					  u32 hash)
{
	struct bb_bpf_lru_node *node = bb_bpf_lru_pop_free(&htab_inner(htab)->lru, hash);
	struct htab_elem *l;
...
static int prealloc_init(struct bpf_htab *htab)
{
	...
	htab->elems = bpfbox_alloc((u64)htab->elem_size * num_entries);
...
skip_percpu_elems:
	if (htab_is_lru(htab))
		err = bb_bpf_lru_init(&htab_inner(htab)->lru,
	...
	if (htab_is_lru(htab))
		bb_bpf_lru_populate(&htab_inner(htab)->lru, htab->elems,
				 offsetof(struct htab_elem, lru_node),
...
static void prealloc_destroy(struct bpf_htab *htab)
{
	...
	if (htab_is_lru(htab))
		bb_bpf_lru_destroy(&htab_inner(htab)->lru);
	else
...
static int alloc_extra_elems(struct bpf_htab *htab)
{
	...
	/* 			    GFP_USER | __GFP_NOWARN); */
	pptr = (struct htab_elem __bpfbox **) bpfbox_alloc_pcpu(sizeof(struct htab_elem*));
	if (!pptr)
		return -ENOMEM;
...
static struct bpf_map *htab_map_alloc(union bpf_attr *attr)
{
	...
	htab->map.map_inner->value_size = htab->map.value_size;
	htab->map.map_inner->map_type = htab->map.map_type;
	htab->map.map_inner->map_flags = htab->map.map_flags;
	...
	htab_inner(htab)->n_buckets = htab->n_buckets;
	...
	err = -ENOMEM;
	htab->buckets = bpfbox_alloc(htab->n_buckets * sizeof(struct bucket));
	htab_inner(htab)->buckets = bpf_box_ptr(htab->buckets);
	...
	map_locked = (local_t **) bpfbox_alloc_pcpu(HASHTAB_MAP_LOCK_COUNT * sizeof(local_t));
	if (!map_locked)
		goto free_map_locked;
	htab_inner(htab)->map_locked = box(map_locked);
	if (htab->map.map_flags & BPF_F_ZERO_SEED)
		htab->hashrnd = 0;
	else
		htab->hashrnd = get_random_u32();
	htab_inner(htab)->hashrnd = htab->hashrnd;
...
free_map_locked:
	...
	if (map_locked)
		bpfbox_free(map_locked);
	bpfbox_free(htab->buckets);
...
static inline struct bb_hlist_nulls_head *select_bucket(struct bpf_htab *htab, u32 hash)
{
 ...
 */
static struct htab_elem *lookup_nulls_elem_raw(struct bb_hlist_nulls_head *head,
					       u32 hash, void *key,
...
void __bpfbox *__htab_map_lookup_elem(struct bpf_map_inner __bpfbox *_map_inner,
				      void __bpfbox *_key);

static void *_htab_map_lookup_elem(struct bpf_map *map, void *key)
{
	return bpf_unbox_or_null_ptr(__htab_map_lookup_elem(bpf_box_ptr(map->map_inner), bpf_box_ptr(key)));
}
				   ...
				   struct bpf_insn *insn_buf)
{
	...
	BUILD_BUG_ON(!__same_type(&__htab_map_lookup_elem,
		     (void *(*)(struct bpf_map_inner __bpfbox *map, void __bpfbox *key))NULL));
	*insn++ = BPF_EMIT_CALL(__htab_map_lookup_elem);
	*insn++ = BPF_JMP_IMM(BPF_JEQ, ret, 0, 6);
	*insn++ = BPF_ST_BOXMEM();
	*insn++ = BPF_LDX_MEM(BPF_B, ref_reg, ret,
			      offsetof(struct htab_elem, lru_node) +
			      offsetof(struct bb_bpf_lru_node, ref));
	*insn++ = BPF_JMP_IMM(BPF_JNE, ref_reg, 0, 2);
	*insn++ = BPF_ST_BOXMEM();
 ...
 */
static bool htab_lru_map_delete_node(void *arg, struct bb_bpf_lru_node __bpfbox *_node)
{
	struct bb_bpf_lru_node *node = unbox(_node);
	struct bpf_htab *htab = arg;
	struct htab_elem __bpfbox *l = NULL;
	struct htab_elem *tgt_l;
	...
	bb_hlist_nulls_for_each_entry_rcu(l, n, head, hash_node)
		if (unbox(l) == tgt_l) {
			bb_hlist_nulls_del_rcu(&l->hash_node);
			check_and_free_fields(htab, unbox(l));
			break;
		}

	htab_unlock_bucket(htab, b, tgt_l->hash, flags);

	return unbox(l) == tgt_l;
}
...
find_first_elem:
	...
	for (; i < htab->n_buckets; i++) {
		...
		next_l = bb_hlist_nulls_entry_safe(BB_READ_ONCE(head->first),
					  struct htab_elem, hash_node);
		if (next_l) {
					 ...
					 struct htab_elem *old_elem)
{
	...
	struct htab_elem *l_new, __bpfbox **pl_new;
	void __percpu *pptr;
...
static void htab_lru_push_free(struct bpf_htab *htab, struct htab_elem *elem)
{
	...
	bb_bpf_lru_push_free(&htab_inner(htab)->lru, &elem->lru_node);
}

int __htab_lru_map_update_elem(struct bpf_map_inner __bpfbox *_map_inner,
				      void __bpfbox *_key, void __bpfbox *_value,
				      u64 map_flags);
					     ...
					     bool onallcpus)
{
	...
	if (l_old) {
		bb_bpf_lru_node_set_ref(&l_old->lru_node);
...
err:
	htab_unlock_bucket(htab, b, hash, flags);
	if (l_new)
		bb_bpf_lru_push_free(&htab_inner(htab)->lru, &l_new->lru_node);
	return ret;
}
...
static void delete_all_elements(struct bpf_htab *htab)
{
	...
	migrate_disable();
	for (i = 0; i < htab->n_buckets; i++) {
		struct bb_hlist_nulls_head *head = select_bucket(htab, i);
		struct bb_hlist_nulls_node *n;
		struct htab_elem *l;

		bb_hlist_nulls_for_each_entry_safe(l, n, head, hash_node) {
			bb_hlist_nulls_del_rcu(&l->hash_node);
			htab_elem_free(htab, l);
		}
	}
...
static void htab_free_malloced_timers(struct bpf_htab *htab)
{
	...
	for (i = 0; i < htab->n_buckets; i++) {
		...
		bb_hlist_nulls_for_each_entry(l, n, head, hash_node) {
...
static void htab_map_free(struct bpf_map *map)
{
	...
	bpfbox_free_pcpu(htab->extra_elems);
	bpfbox_free(htab->buckets);
	bpf_mem_alloc_destroy(&htab->pcpu_ma);
	bpf_mem_alloc_destroy(&htab->ma);
	if (htab->use_percpu_counter)
		percpu_counter_destroy(&htab->pcount);
	bpfbox_free_pcpu(unbox(htab_inner(htab)->map_locked));
	bpfbox_free(htab_inner(htab));
	lockdep_unregister_key(&htab->lockdep_key);
	bpf_map_area_free(htab);
}
...
again_nocopy:
	...
	bb_hlist_nulls_for_each_entry_safe(l, n, head, hash_node) {
		...
		if (do_delete) {
			...
			/* bb_bpf_lru_push_free() will acquire lru_lock, which
			 * may cause deadlock. See comments in function
			 * prealloc_lru_pop(). Let us do bb_bpf_lru_push_free()
			   ...
			   struct htab_elem *prev_elem)
{
	...
	if (prev_elem) {
		 ...
		 */
		n = BB_READ_ONCE(prev_elem->hash_node.next);
		elem = bb_hlist_nulls_entry_safe(n, struct htab_elem, hash_node);
		if (elem)
			return elem;
	...
	for (i = bucket_id; i < htab->n_buckets; i++) {
		...
		count = 0;
		head = &b->head;
		bb_hlist_nulls_for_each_entry_rcu(elem, n, head, hash_node) {
			if (count >= skip_elems) {
				info->bucket_id = i;
...
const struct bpf_map_ops htab_map_ops = {
	...
	.bpfbox_map_update_elem = __htab_map_update_elem,
...
const struct bpf_map_ops htab_lru_map_ops = {
	...
	.bpfbox_map_update_elem = __htab_lru_map_update_elem,
