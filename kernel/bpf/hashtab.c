...
static void __bpfbox *__htab_map_lookup_elem(struct bpf_map_inner __bpfbox *_map_inner,
					void __bpfbox *_key)
{
...
{
	BUG();
	__htab_map_update_elem(bpf_box_ptr(map->map_inner), bpf_box_ptr(key), bpf_box_ptr(value), map_flags);
}
