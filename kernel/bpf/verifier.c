				      ...
				      int off, int size, int dst_regno)
{
	...
	if (is_spilled_reg(&reg_state->stack[spi])) {
			...
			if (!(off % BPF_REG_SIZE) && size == spill_size) {
				...
				for (i = 0; i < size; i++) {
					...
					verbose(env, "invalid read from stack off %d+%d size %d\n",
						off, i, size);
					reads_secret_data = true;
					break;
				}
				if (reads_secret_data) {
		...
		if (reads_secret_data) {
			...
			mark_reg_read(env, reg, reg->parent, REG_LIVE_READ64);
			if (dst_regno >= 0)
				mark_reg_stack_read(env, reg_state, off,
						    off + size, dst_regno);
		}
	}
	return 0;
...
BTF_ID(func, bpf_map_lookup_u32_by_value)
BTF_ID(func, bpf_map_lookup_elem_by_value)
...
static int do_check_insn(struct bpf_verifier_env *env, struct bpf_insn *insn,
	...
	if (class == BPF_ALU || class == BPF_ALU64) {
		...
		if (opcode == BPF_CALL) {
			...
			if (err)
				return err;
			return process_bpf_exit(env, prev_insn_idx, pop_log, do_print_state);
		} else {
